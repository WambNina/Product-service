...
const ProductVariant = sequelize.define(
  "ProductVariant",
  {
    ...
    attributes: {
      ...
      defaultValue: {},
      comment:
        "Tous les attributs avec pricing: {color, size, capacity, weight, etc.}",
    },
    ...
    price_adjustment: {
      ...
      defaultValue: 0.0,
      comment:
        "DEPRECATED - Utiliser attributes.{color|size|weight}.priceImpact",
    },
    ...
    calculated_price: {
      ...
      allowNull: true,
      comment:
        "Prix auto-calculé: base + colorImpact + sizeImpact + weightImpact",
    },
    ...
    final_price: {
      ...
      allowNull: false,
      comment:
        "Prix effectif: COALESCE(override_price, calculated_price, price, 0)",
    },
    ...
    pricing_metadata: {
      ...
      defaultValue: {},
      comment:
        "Détails du calcul: {basePrice, colorImpact, sizeImpact, weightImpact, totalImpact}",
    },
  ...
  },
  {
    ...
    hooks: {
      beforeValidate: (variant) => {
        // Only sync attributes if they exist, don't touch other fields
        if (variant.attributes && typeof variant.attributes === "object") {
          if (variant.attributes.color?.value)
            variant.color = variant.attributes.color.value;
          if (variant.attributes.size?.value)
            variant.size = variant.attributes.size.value;
          if (variant.attributes.capacity?.value)
            variant.capacity = variant.attributes.capacity.value;
          if (variant.attributes.weight?.value)
            variant.weight = variant.attributes.weight.value;
        }

        // Only calculate price if we have the necessary data
        if (variant.base_price || variant.attributes) {
...
function calculateFinalPrice(variant) {
  ...
  let colorImpact = parseFloat(variant.attributes?.color?.priceImpact) || 0;
  let sizeImpact = parseFloat(variant.attributes?.size?.priceImpact) || 0;
  let weightImpact = parseFloat(variant.attributes?.weight?.priceImpact) || 0;
  let capacityImpact =
    parseFloat(variant.attributes?.capacity?.priceImpact) || 0;

  const calculatedPrice =
    basePrice + colorImpact + sizeImpact + weightImpact + capacityImpact;

  variant.calculated_price = Math.round(calculatedPrice * 100) / 100;

  // models/ProductVariant.js - Update calculateFinalPrice function
  function calculateFinalPrice(variant) {
    let basePrice = 0;
    ...
    let weightImpact = parseFloat(variant.attributes?.weight?.priceImpact) || 0;
    let capacityImpact =
      parseFloat(variant.attributes?.capacity?.priceImpact) || 0; // 🆕 ADD THIS
    ...
    variant.pricing_metadata = {
      ...
      capacityImpact: capacityImpact, // 🆕 ADD THIS
      totalImpact: colorImpact + sizeImpact + weightImpact + capacityImpact, // 🆕 INCLUDE capacity
...
ProductVariant.associate = (models) => {
  ...
  ProductVariant.hasMany(models.ProductVariantPrice, {
    foreignKey: "variant_id",
    as: "priceHistory",
  });
};
